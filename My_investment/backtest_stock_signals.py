# -*- coding: utf-8 -*-
"""종목별 매수·매도 신호 10가지 1년 백테스트 (익절+3% / 손절-3%)"""
import pandas as pd
...
def run_backtest(hist, mode='dual'):
    ...
    start_idx = hist.index.get_loc(hist.tail(252).index[0])

    cash, shares, avg_cost = INITIAL, 0, 0.0
    n_buy = n_sell = n_hold = 0
    trades, daily_log = [], []

    for i in range(start_idx, len(hist) - 1):
        day, next_day = hist.index[i], hist.index[i + 1]
        ...
        action, reason, sold = '관망', f'매수{b_hit}/매도{s_hit}', False
        ...
        if sold:
            ...
            do_buy = (mode == 'buy_only' and b_hit >= 4) or (
                mode in ('dual', 'dual_no_tp') and b_hit >= 4 and s_hit <= 2)
            if do_buy:
                qty, cash = buy_shares(cash, exec_price)
                if qty > 0:
                    shares, avg_cost = qty, exec_price
                    action, reason = '매수', f'매수{b_hit}/매도{s_hit}'
                    trades.append({
...
def run_report(ticker, name, out_prefix):
    hist = yf.Ticker(ticker).history(period='2y')
    hist.index = hist.index.tz_localize(None) if hist.index.tz else hist.index
    start_date = hist.tail(252).index[0].strftime('%Y-%m-%d')
    end_date = hist.index[-1].strftime('%Y-%m-%d')

    r_dual = run_backtest(hist, 'dual')
    r_dual_nt = run_backtest(hist, 'dual_no_tp')
    r_old = run_backtest(hist, 'buy_only')
    ...
    print('=' * 85)
    print(f'  {name}({ticker.replace(".KS", "")}) 매수·매도 신호 통합 1년 백테스트')
    print(f'  기간: {start_date} ~ {end_date}  |  초기: {INITIAL:,}원')
    print('=' * 85)
    print('\n[규칙] 매수4+ & 매도2- → 매수 | 매도4+ or +3%익절 or -3%손절 → 매도')
    ...
    print('  ★ 시나리오 A 상세')
    print('=' * 85)
    print(f"  최종 평가액   : {r['final']:,.0f}원 ({r['return_pct']:+.1f}%)")
    print(f"  손익          : {r['final']-INITIAL:+,.0f}원")
    print(f"  현재 보유     : 현금 {r['cash']:,.0f}원 + {r['shares']}주 × {r['last_close']:,.0f}원")
    print(f"  매수/매도/관망: {r['n_buy']} / {r['n_sell']} / {r['n_hold']}일")
    print(f"  매도 승률     : {r['win_rate']:.0f}% ({r['wins']}/{r['sells']}회)")
    print(f"  vs Buy&Hold   : {r['return_pct']-bh_return:+.1f}%p  (B&H {bh_return:+.1f}%)")

    if len(r['trades']) > 0:
        sells = r['trades'][r['trades']['side'] == 'SELL']
        print('\n  [매도 사유별]')
        for pfx in ['매도신호', '익절', '손절']:
            sub = sells[sells['reason'].str.startswith(pfx)]
            if len(sub):
                print(f'    {pfx}: {len(sub)}회  평균 {sub["pnl_pct"].mean():+.1f}%')
    ...
    if len(r['trades']) > 0:
        ...
        for _, t in r['trades'].tail(10).iterrows():
            ...
            print(f"    {pd.Timestamp(t['date']).strftime('%Y-%m-%d')}  {t['side']:<4} {int(t['qty']):>3}주 "
                  f"@ {t['price']:>8,.0f}  {t['reason']}{pnl}")

    base = r'c:\Users\seaho\My project\My_investment'
    r['trades'].to_csv(f'{base}\\{out_prefix}_trades.csv', index=False, encoding='utf-8-sig')
    r['daily'].to_csv(f'{base}\\{out_prefix}_daily.csv', index=False, encoding='utf-8-sig')
    print(f'\n  저장: {out_prefix}_trades.csv, {out_prefix}_daily.csv')

    return r_dual, bh_return


if __name__ == '__main__':
    ticker = sys.argv[1] if len(sys.argv) > 1 else '105560.KS'
    name = sys.argv[2] if len(sys.argv) > 2 else 'KB금융'
    prefix = sys.argv[3] if len(sys.argv) > 3 else 'kb_backtest'
    run_report(ticker, name, prefix)
